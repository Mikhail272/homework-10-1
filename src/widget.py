...
def mask_account_card(type_and_number: str) -> str:
    ...
    if not type_and_number or len(type_and_number.strip().split()) < 2:
        raise ValueError(
            "Некорректный ввод: должно быть указано название карты или счета и номер."
        )
    ...
    if (
        not digit_result.isdigit() or len(digit_result) < 10
    ):  # Проверка на цифры и минимальную длину
        raise ValueError(
            "Номер карты или счета должен содержать только цифры и иметь корректную длину."
        )
